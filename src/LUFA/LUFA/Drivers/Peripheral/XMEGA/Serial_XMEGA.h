	...
	/* Public Interface - May be used in end-application: */
		...
		/* Inline Functions: */
			...
			static inline void Serial_Init(USART_t* const USART,
			                               ...
			                               const bool DoubleSpeed, const bool RXDREinterruptsEnable) ATTR_NON_NULL_PTR_ARG(1);
			static inline void Serial_Init(USART_t* const USART,
			                               const uint32_t BaudRate,
			                               const bool DoubleSpeed, const bool RXDREinterruptsEnable)
			{
				uint16_t BaudValue = 131;//(DoubleSpeed ? SERIAL_2X_UBBRVAL(BaudRate) : SERIAL_UBBRVAL(BaudRate));

				USART->BAUDCTRLB = 0xF0; //scale = -3
				USART->BAUDCTRLA = 33;

				if (RXDREinterruptsEnable)
				{
					//Setup Interrupts
